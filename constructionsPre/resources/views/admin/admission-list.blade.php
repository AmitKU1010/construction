            ...
            <div class="box-footer">
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td>{{ $student['name']}}</td>
                  <td>F: {{ $student['father_name']}} <br> M: {{ $student['mother_name']}}</td>
                  <td>{{$student['status']}}</td>
                  <td>{{ $student['gender']}}</td>
                  <td>{{ $student['dob']}}</td>
                  <td>{{ $student['admission_class']}}</td>
                  <td>
                  @if(isset($student['status']) && $student['status'] == 'NEWJOIN' && $student['readmission_id'] =='')
                    <a href="{{url('/')}}/admission/{{ $student['id'] }}" title="Edit Student Details" class="btn btn-success btn-xs"><i class="fa fa-edit"></i></a> 
                    
                    <a href="{{url('/')}}/admission-delete/{{ $student['id']}}" title="Delete Student" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fa fa-trash"></i></a> 
                    <a href="{{url('/')}}/payment/{{ $student['id']}}" title="Make Payment" class="btn btn-primary btn-xs" title="Make Payment" onclick="return confirm('Are you sure you want to proceed to payment?');"><i class="fa fa-inr"></i></a> 
                    <a href="{{url('/')}}/readmission/{{ $student['id'] }}" title="Readmission" class="btn btn-default btn-xs"><i class="fa fa-step-forward"></i></a>
                    
                  @elseif(isset($student['status']) && $student['status'] == 'COMPLETED' && $student['readmission_id'] != '')
                    {!! '<span class="text-purple">Promoted</span>' !!}
                  @elseif(isset($student['status']) && $student['status'] == 'TRANSFER')
                    {!! '<span class="text-danger">Transfered</span>' !!}
                  <a href="{{url('/')}}/transfer/{{ $student['id']}}" target="_blank" title="Transfer Certificate Download" class="btn btn-info btn-xs"><i class="fa fa-plane"></i></a>
                  @endif
