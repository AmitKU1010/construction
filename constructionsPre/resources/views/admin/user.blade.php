  ...
  .box-title{
    margin-left: 12px;
      ...
      <div class="box box-default">
        ...
        @endif
             {{ Form::open(['url' => 'admin/employee-save', 'files' => true]) }} 
             {{ csrf_field() }}
                                ...
                                <div class="form-group {{ $errors->has($CFkey) ? 'has-error' : ''}}">
                                  ...
                                  <div class="input text">
                                    @if($CFvalue['type'] == 'text')
                                      {{ Form::text($CFkey, isset($$CFkey) ? $$CFkey :'', ['class' => 'form-control input-md '.$class.' ', 'id' => isset($CFvalue['id']) ? $CFvalue['id']: '', 'style' => isset($CFvalue['style']) ? $CFvalue['style']: '', 'placeholder' => $CFvalue['label'], 'autocomplete' => 'off', isset($CFvalue['readonly']) && $CFvalue['readonly'] == true ? 'readonly' : ''  ]) }}
                                    @elseif($CFvalue['type'] == 'select')
                                      {{ Form::select($CFkey, $CFvalue['value'],  isset($$CFkey) ? $$CFkey :'', ['class' => 'form-control input-md '.$class.' ', 'style' => isset($CFvalue['style']) ? $CFvalue['style']: '', 'placeholder' => '- Please Select -' ]) }}
    ...
    <script>
      $(document).ready(function(){
        //Perday salary calculation
        $(".net_salary").keyup(function(){
          var net_salary = $(".net_salary").val();
          var perdaysalary = parseFloat(net_salary) / 30;
