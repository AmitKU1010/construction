  ...
  .box-title{
    margin-left: 12px;
      ...
      <div class="box box-default">
        ...
        @endif
             {{ Form::open(['url' => 'admin/voucher-save']) }} 
             {{ csrf_field() }}
             {{ Form::hidden('id', isset($id) ? $id :'', []) }}
                <div class="row">
                  <h4 class="box-title text-purple">Enter Voucher Details</h4>
                              ...
                              <div class="form-group {{ $errors->has($CFkey) ? 'has-error' : ''}}">
                                ...
                                <div class="input text">
                                  ...
                                  @elseif($CFvalue['type'] == 'checkbox')
                                    {{ Form::checkbox($CFkey, isset($$CFkey) ? $$CFkey :'', ['id' => '', 'class' => 'form-control']) }} {{ $CFvalue['label'] }}
         ...
         $(document).ready(function() {
          $('.overlay').hide();
            ...
            $(".employee_id").change(function(event){
              var emplname = $('.employee_id :selected').text();
              $(".pay_to").val(emplname);
            });
            ...
            $(".flow_type ").change(function(){
                ...
                if(flow_type == 'OPNBAL'){
                  ...
                  var person = $('.employee_id :selected').text();
                  $(".pay_to").val(person);
                  $('.pay_to').prop('readonly', true);
                }else{
                  $('.employee_id').val('');
