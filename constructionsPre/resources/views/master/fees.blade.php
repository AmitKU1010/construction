                              ...
                              <div class="form-group {{ $errors->has($CFkey) ? 'has-error' : ''}}">
                                ...
                                <div class="input text">
                                  ...
                                  @elseif($CFvalue['type'] == 'select')
                                    {{ Form::select($CFkey, $CFvalue['value'],  isset($$CFkey) ? $$CFkey :'', ['class' => 'form-control input-md '.$class.' ', 'placeholder' => '--Select Option--' ]) }}
                                  @elseif($CFvalue['type'] == 'file')
                                    {{ Form::file($CFkey, ['id' => '', 'class' => 'form-control']) }}
                                  @elseif($CFvalue['type'] == 'checkbox')
                                    {{ Form::checkbox($CFkey, '1', false, ['id' => 'exampleInputFile']) }} Yes.
                                  @endif
                                </div>
                                <p class="help-block">
        ...
        <div id="admission-list">
            <table class="table table-responsive table-bordered">
                ...
                <tbody>
                  ...
                  <tr class="{{$feesdata['is_secret'] == 1 ? '' : 'danger'}}">
                    <td>{{ $data['i'] }}</td>
                    ...
                    <td>
                      <div class="" style="white-space: nowrap">
                        <span data-toggle="modal" data-target="#addStockModal_{{$feesdata['id']}}" class="btn btn-success btn-xs"><i class="fa fa-plus"></i></span> 
                          {!! $feesdata['quantity'] > 0 ? $feesdata['quantity'] : '<span class="text-danger">0</span>' !!}
                        <span title="View Stock addition Hsitory" data-toggle="modal" data-target="#stockModalHistory_{{$feesdata['id']}}" class="btn btn-info btn-xs"><i class="fa fa-history"></i></span>
                      </div>
                    </td>
                    <td>{{ $feesdata['amount'] }}</td>
                    <td>
                      <ol>
                        <li>Inv #: {{ $feesdata['invoice_no'] }}</li>
                        <li>Inv Dt.: {{ $feesdata['invoice_date'] }}</li>
                        <li>Amount: {{ $feesdata['total_amount'] }}</li>
                        <li>Disc.: {{ $feesdata['discount'] }}</li>
                        <li>Grand Total : {{ $feesdata['grand_total'] }}</li>
                      </ol>
                    </td>
                    ...
                    <!-- Modal -->
                      <div id="addStockModal_{{$feesdata['id']}}" class="modal fade" role="dialog">
                        <div class="modal-dialog">
                          ...
                          <div class="modal-content">
                            <div class="modal-header">
                              <button type="button" class="close" data-dismiss="modal">&times;</button>
                              <h4 class="modal-title">Add More Stocks</h4>
                            </div>
                            <div class="modal-body">
                          {{ Form::open(['url' => 'fees/stock/add', 'form' => '1']) }} 
                          {{ csrf_field() }}
                          {{ Form::hidden('fees_master_id', isset($feesdata['id']) ? $feesdata['id'] :'', []) }}
                          <div class="row">
                              @foreach($customFields['modal'] as $CFkey => $CFvalue)
                                @php $class = isset($CFvalue['class']) ? $CFvalue['class'] : ''; @endphp
                                      <div class="col-sm-4">
                                        <div class="form-group {{ $errors->has($CFkey) ? 'has-error' : ''}}">
                                                <label for="exampleInputFile">{{ $CFvalue['label'] }} 
                          ...
                          <center>
                            <div class="box-footer">
                              {{ Form::submit($formButton, array('name' => 'more-stock-add', 'class' => 'btn btn-success')) }}
                              {{ Form::reset('Reset', array('class' => 'btn btn-warning')) }}
                            </div>
                          </center>
                        {{ Form::close() }}
                      ...
                      {{-- Histiry Modal --}}
                      <div id="stockModalHistory_{{$feesdata['id']}}" class="modal fade" role="dialog">
                        <div class="modal-dialog modal-lg">
                          ...
                          <div class="modal-content">
                            <div class="modal-header">
                              <button type="button" class="close" data-dismiss="modal">&times;</button>
                              <h4 class="modal-title">Stock Addition History</h4>
                            </div>
                            <div class="modal-body">
                          {{ Form::open(['url' => 'fees/stock/add', 'form' => '1']) }} 
                          {{ csrf_field() }}
                          ...
                          <div class="row">
                            <table class="table table-responsive table-bordered">
                              ...
                              <tbody>
                                @foreach($feesdata['stock_history'] as $key => $history)
                                <tr>
                                <td>{{$key+1}}</td>
                                  ...
                                  <td>{{ date('d/M/Y', strtotime($history['invoice_date'])) }}</td>
                                  <td>{{ $history['total_amount'] }}</td>
                                  <td>{{ $history['discount'] }}</td>
                                  <td>{{ $history['grand_total'] }}</td>
                                </tr>
                                @endforeach
                              </tbody>
                            </table>
